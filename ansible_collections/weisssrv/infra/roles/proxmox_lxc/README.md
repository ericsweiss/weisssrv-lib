...
| Per-container DNS — `nameserver` (`proxmox_lxc_nameserver`) and `searchdomain` (`proxmox_lxc_searchdomain`) | **Reconciled** on existing containers via an idempotent `pct set --nameserver --searchdomain` (only when a non-empty `proxmox_lxc_nameserver` is set). `pct set` stages the change as **pending**, which Proxmox applies on the next container restart — a running container is not disrupted and nothing is rebooted. This matters because Proxmox rewrites the container's `/etc/resolv.conf` from the stored nameserver on every start, so a stale value would otherwise keep clobbering the container's resolvers (e.g. after a subnet renumber) until a deploy repaired it. |
